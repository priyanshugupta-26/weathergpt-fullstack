...
class MLFeatureBuilder:
    ...
    def build_features_from_obs(
        ...
        # 1. Base telemetry
        lat = float(current_obs.get("latitude") or 25.5941)
        lon = float(current_obs.get("longitude") or 85.1376)
        temp_val = current_obs.get("temperature") if current_obs.get("temperature") is not None else current_obs.get("temperature_2m")
        temp = float(temp_val if temp_val is not None else 27.0)
        rh_val = current_obs.get("humidity") if current_obs.get("humidity") is not None else current_obs.get("relative_humidity_2m")
        rh = float(rh_val if rh_val is not None else 60.0)
        dew_val = current_obs.get("dew_point") if current_obs.get("dew_point") is not None else current_obs.get("dew_point_2m")
        dew = float(dew_val if dew_val is not None else (temp - ((100 - rh) / 5)))
        app_val = current_obs.get("feels_like") if current_obs.get("feels_like") is not None else current_obs.get("apparent_temperature")
        app_temp = float(app_val if app_val is not None else (temp + 1.5))
        precip = float(current_obs.get("precipitation") or 0.0)
        rain_val = current_obs.get("rainfall") if current_obs.get("rainfall") is not None else current_obs.get("rain")
        rain = float(rain_val if rain_val is not None else precip)
        press_val = current_obs.get("surface_pressure") if current_obs.get("surface_pressure") is not None else current_obs.get("pressure")
        press = float(press_val if press_val is not None else 1005.0)
        et0 = float(current_obs.get("et0_fao_evapotranspiration") or 0.2)
        vpd = float(current_obs.get("vapour_pressure_deficit") or 1.2)
        ws_val = current_obs.get("wind_speed") if current_obs.get("wind_speed") is not None else current_obs.get("wind_speed_10m")
        wind_speed = float(ws_val if ws_val is not None else 10.0)
        wd_val = current_obs.get("wind_direction") if current_obs.get("wind_direction") is not None else current_obs.get("wind_direction_10m")
        wind_dir = float(wd_val if wd_val is not None else 90.0)
        wg_val = current_obs.get("wind_gust") if current_obs.get("wind_gust") is not None else current_obs.get("wind_gusts_10m")
        wind_gust = float(wg_val if wg_val is not None else (wind_speed * 1.4))
        st_val = current_obs.get("soil_temperature") if current_obs.get("soil_temperature") is not None else current_obs.get("soil_temperature_0_to_7cm")
        soil_temp = float(st_val if st_val is not None else temp)
        sm_val = current_obs.get("soil_moisture") if current_obs.get("soil_moisture") is not None else current_obs.get("soil_moisture_0_to_7cm")
        soil_moist = float(sm_val if sm_val is not None else 0.3)
        shortwave = float(current_obs.get("shortwave_radiation") or 400.0)
        dni = float(current_obs.get("direct_normal_irradiance") or 300.0)
        diffuse = float(current_obs.get("diffuse_radiation") or 100.0)
        cloud = float(current_obs.get("cloud_cover") or 20.0)
